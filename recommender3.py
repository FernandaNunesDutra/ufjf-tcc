...
def computeRecommendations2(predictions):
    recommendations = np.zeros((predictions.shape[0],3,),dtype='int')
    for userIndex, userRow in enumerate(predictions):
...
def main():
    ...
    for trainIndex, testIndex in kf.split(userData):
        ...
        recommendations = computeRecommendations(predictions)
        ...
        for i, ratings in enumerate(userItemMatrixTest):
            ...
            precision += metrics.precision_score(truth, predicted, labels=[1,-1], pos_label=1, average='binary')
            recall += metrics.recall_score(truth, predicted, labels=[1,-1], pos_label=1, average='binary')
        precision = precision/userItemMatrixTest.shape[0]
        recall = recall/userItemMatrixTest.shape[0]
        print 'average precision: ', precision
        print 'average recall: ', recall
        print '------------'
        #print metrics.confusion_matrix(pred, predictions)
